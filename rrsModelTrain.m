...
% 4. n_permutations: the numberof times you want to do the entire
...
function [coefficients, intercepts, summary_gofs, all_gofs] = rrsModelTrain(daph, pft, pft_index, n_permutations, max_pcs, k, mdl_pick_metric, output_file_name)
    ...
    if strcmp(pft_index, 'pigment') == 0 && strcmp(pft_index, 'EOFs') == 0 && strcmp(pft_index, 'compositions') == 0
        ...
        return
    elseif strcmp(mdl_pick_metric,'R2') == 0 && strcmp(mdl_pick_metric,'RMSE') == 0 && strcmp(mdl_pick_metric,'avg') == 0 && strcmp(mdl_pick_metric,'med') == 0 && strcmp(mdl_pick_metric,'ens') == 0 && strcmp(mdl_pick_metric,'bias') == 0 && strcmp(mdl_pick_metric,'MAE') == 0
    ...
    for i = 1:n_permutations
        ...
        for j = 1:k
            ...
            for l = 1:size(CV_AFs_train,2)
                ...
                all_bias(1:length(CV_valid_pigs),l) = (CV_modeled_pigs-CV_valid_pigs);
                bias(l) = mean(all_bias(:,l));
                MAE(l) = mean(abs(all_bias(:,l)));
                ...
                ensemble(l) = (1-R2s(l)+RMSEs(l))/100;
            end
            ...
            elseif strcmp(mdl_pick_metric, 'ens') == 1
                ii = find(ensemble == min(ensemble));
                if length(ii) > 1
                    ii = min(ii);
                end
                n_modes_to_use(j) = ii; % for selecting based on minimizing ensenble statistic
            elseif strcmp(mdl_pick_metric, 'bias') == 1
                ii = find(bias == min(bias));
                if length(ii) > 1
                    ii = min(ii);
                end
                n_modes_to_use(j) = ii; % for selecting based on minimizing bias
            elseif strcmp(mdl_pick_metric, 'MAE') == 1
                ii = find(MAE == min(MAE));
                if length(ii) > 1
                    ii = min(ii);
                end
                n_modes_to_use(j) = ii; % for selecting based on minimizing mean abs error    
            end
        ...
        mae(i) = mean(abs(modeled_pigs - pigs_validate));
        % display permutation number here...
        disp(['hey dude, im doing good. im on permutation # ',num2str(i)]);
    ...
    summary_gofs = [mean(R2s_final),std(R2s_final),mean(RMSEs_final),std(RMSEs_final),mean(avg_pct_error),std(avg_pct_error),...
        mean(med_pct_error),std(med_pct_error),mean(pct_bias),std(pct_bias),mean(mae),std(mae)];
    ...
    summary_gofs.Properties.VariableNames = {'Mean_R2','SD_R2','Mean_RMSE','SD_RMSE',...
        ...
        'Mean_pct_bias','SD_pct_bias','Mean_MAE','SD_MAE'};
    ...
    all_gofs = struct('R2s',R2s_final,'RMSEs',RMSEs_final,'mean_pct_error',avg_pct_error,...
        'median_pct_error',med_pct_error,'pct_bias',pct_bias, 'all_pct_errors', pct_errors,'all_mae',mae);
